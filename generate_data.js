...
// Generate Timezones with offsets
const timezones = Intl.supportedValuesOf('timeZone').map(tz => {
  ...
  // Extract offset to sort by it
  const getOffsetNum = (str) => {
    const match = str.match(/GMT([+-]\d+)(?::(\d+))?/);
    if (!match) return 0;
    return parseInt(match[1]) * 60 + (parseInt(match[2]) || 0) * (match[1].startsWith('-') ? -1 : 1);
  };
  return getOffsetNum(a.label) - getOffsetNum(b.label) || a.label.localeCompare(b.label);
});

// A comprehensive list of country dial codes
const countryCodes = [
...
export const countryCodes = ${JSON.stringify(countryCodes, null, 2)};
`;

fs.writeFileSync('/Users/akshatsingh/Storage/Coding/TutorProfile/akshat-tutor-portfolio/app/data.ts', fileContent.trim());
